...
void hod::getnewpar(cosmo &p, hodpars &q, gsl_rng *&r, double * cho, int * p2v, int Nvary)
{
    double *gran;

    gran=new double[Nvary];

    /// Generate Nvary random numbers
    for(int i=0;i<Nvary;i++)
    {
        gran[i]=gsl_ran_gaussian(r,1.0);
    }
    ...
    for(int i=0;i<Nvary;i++)
    {
        ...
        {
            add=add+cho[i*Nvary+j]*gran[j];
        }
        changepar(p,q,p2v[i],add);
    }
...
void hod::getnewpar(cosmo &p, hodpars &q, cosmo pv, hodpars qv, gsl_rng *&r)
{

    /// Cosmology parameters
    if(pv.Om0!=0.0){
        p.Om0=p.Om0+gsl_ran_gaussian(r,pv.Om0);
    }
    if(pv.w0!=0.0){
        p.w0=p.w0+gsl_ran_gaussian(r,pv.w0);
    }
    if(pv.wa!=0.0){
        p.wa=p.wa+gsl_ran_gaussian(r,pv.wa);
    }
    if(pv.Omk!=0.0){
        p.Omk=p.Omk+gsl_ran_gaussian(r,pv.Omk);
    }
    if(pv.hval!=0.0){
        p.hval=p.hval+gsl_ran_gaussian(r,pv.hval);
    }
    if(pv.Omb!=0.0){
        p.Omb=p.Omb+gsl_ran_gaussian(r,pv.Omb);
    }
    if(pv.s8!=0.0){
        p.s8=p.s8+gsl_ran_gaussian(r,pv.s8);
    }
    if(pv.nspec!=0.0){
        p.nspec=p.nspec+gsl_ran_gaussian(r,pv.nspec);
    }
    if(pv.ximax!=0.0){
        p.ximax=p.ximax+gsl_ran_gaussian(r,pv.ximax);
    }
    if(pv.cfac!=0.0){
        p.cfac=p.cfac+gsl_ran_gaussian(r,pv.cfac);
    }
    if(pv.th!=0.0){
        p.th=p.th+gsl_ran_gaussian(r,pv.th);
        printf("Should never happen: theta is varying\n");
        exit(0);
    }

    /// Central HOD parameters
    if(qv.Mmin!=0.0){
        q.Mmin=q.Mmin+gsl_ran_gaussian(r,qv.Mmin);
    }
    if(qv.siglogM!=0.0){
        q.siglogM=q.siglogM+gsl_ran_gaussian(r,qv.siglogM);
    }
    if(qv.Msat!=0.0){
        q.Msat=q.Msat+gsl_ran_gaussian(r,qv.Msat);
    }
    if(qv.alpsat!=0.0){
        q.alpsat=q.alpsat+gsl_ran_gaussian(r,qv.alpsat);
    }
    if(qv.Mcut!=0.0){
        q.Mcut=q.Mcut+gsl_ran_gaussian(r,qv.Mcut);
    }

    if(qv.fac!=0.0){
        q.fac=q.fac+gsl_ran_gaussian(r,qv.fac);
    }
    if(qv.csbycdm!=0.0){
        q.csbycdm=q.csbycdm+gsl_ran_gaussian(r,qv.csbycdm);
        //printf("Should never happen: csbycdm is varying \n");
        //exit(0);
    }
...
/// Change parameters
void hod::changepar(cosmo &p, hodpars &q, int n, double add)
{
    switch (n)
    {
        case 0:
...
/// Change parameters
void hod::changepar(cosmo &p, hodpars &q, int n, double diff, double &denom)
{
    switch (n)
    {
        case 0:
        ...
        case 7:
            denom=p.hval;
            p.hval=p.hval*(1.+diff);
            break;

        case 8:
            denom=p.ximax;
            p.ximax=p.ximax*(1.+diff);
            break;

        case 9:
            denom=p.cfac;
            p.cfac=p.cfac*(1.+diff);
            break;

        case 10:
            denom=q.Mmin;
            q.Mmin=(q.Mmin)*(1.+diff);
            break;

        case 11:
            denom=q.siglogM;
            q.siglogM=(q.siglogM)*(1.+diff);
            break;

        case 12:
            denom=q.Msat;
            q.Msat=q.Msat*(1.+diff);
            break;

        case 13:
            denom=q.alpsat;
            q.alpsat=q.alpsat*(1.+diff);
            break;

        case 14:
            denom=q.Mcut;
            q.Mcut=q.Mcut*(1.+diff);
            break;

        case 15:
            denom=q.csbycdm;
            q.csbycdm=q.csbycdm*(1.+diff);
            break;
